...
void FrameworkFasade::ReplacePayloadRequest(PatternId_t patternId, MessageId_t msgId, void* payload, uint16_t sizeOfPayload)
{
  fi->ReplacePayloadRequest(patternId, msgId, payload,sizeOfPayload);
}

void FrameworkFasade::AddDestinationRequest(PatternId_t patternId, MessageId_t msgId, NodeId_t destArray[MAX_DEST], uint16_t noOfNbrs)
{
  fi->AddDestinationRequest(patternId, msgId, destArray, noOfNbrs);
}
void FrameworkFasade::AddDestinationRequest(PatternId_t patternId, MessageId_t msgId, NodeId_t destArray[MAX_DEST], uint16_t noOfNbrs, LinkComparatorTypeE lcType)
{
  fi->AddDestinationRequest(patternId, msgId, destArray, noOfNbrs, lcType);
}

void FrameworkFasade::CancelDataRequest(PatternId_t patternId, MessageId_t msgId, NodeId_t destArray[MAX_DEST], uint16_t noOfDest)
{
  fi->CancelDataRequest(patternId, msgId, destArray, noOfDest);
}

void FrameworkFasade::DataStatusRequest(PatternId_t patternId, MessageId_t msgId)
{
