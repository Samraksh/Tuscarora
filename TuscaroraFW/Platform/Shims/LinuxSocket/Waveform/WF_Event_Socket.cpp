...
bool WF_Event<WF_RECV_MSG_EVT,WF_RecvMsgParam<uint64_t> >::Invoke (WF_RecvMsgParam<uint64_t> param0){
	...
	uint16_t totalData =  sizeof(WF_RECV_MSG_EVT)+sizeof( FMessageId_t )+sizeof(WF_Message_n64_t)+payloadSize;
	memset(SerializedBuffer,0,totalData);
	...
	int offset = sizeof(WF_RECV_MSG_EVT)+sizeof(FMessageId_t )+sizeof(WF_Message_n64_t);
	if(payloadSize != 0){
