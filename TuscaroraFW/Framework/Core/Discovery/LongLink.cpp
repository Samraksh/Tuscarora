...
void LongLink::Start() {
	...
	numNodes = atoi(getenv("NETWORK_SIZE")) - 1;
		...
		if(msg.GetSource() == numNodes) {
			...
			for(NodeId_t x = iter2 * 25; x < numNodes && x < (iter2 + 1) * 25; x++) {
				...
				if((location.x - locs[x-(iter2*25)].x) * (location.x - locs[x-(iter2*25)].x) + (location.y - locs[x-(iter2*25)].y) * (location.y - locs[x-(iter2*25)].y) < (range * hops) * (range * hops)) {
					PotentialNeighbor *i = nbrs.GetNode(x);
					if(i == 0) {
						for(int w = 2; w < MAX_WAVEFORMS; w++) {
							if(NetworkDiscovery::hasWaveform(w, x)) {
								...
								link.nodeId = x;
								link.waveformId = w;
								i = nbrs.AddNode(link, BEYOND_COMM_PNODE, 0, 1 - (((range * 2)*(range*2)) / ((location.x - locs[x].x)*(location.x-locs[x].x) + (location.y-locs[x].y)*(location.y-locs[x].y))));
								if(del != 0) {
