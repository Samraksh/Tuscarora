...
**/
  ...
  for(uint16_t node_count = 0; node_count < NODES;  node_count++){
   ...
   if(input == 1){
       double total_reception_rate = (double) ((double) total_data_recv / (double)(f_summary.total_valid_sent));
       cout << "total_data_recv " << total_data_recv << "total_valid_sent " << f_summary.total_valid_sent << endl;
       ...
       expected_broadcast_recv = total_data_recv / expected_broadcast_recv; 
       if(expected_broadcast_recv > 0.7){
           cout << "Broadcast Passed: Total_reception_rate is " << expected_broadcast_recv << endl;
       }else{
