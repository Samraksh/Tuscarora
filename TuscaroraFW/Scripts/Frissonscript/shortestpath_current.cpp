...
/**************************
  current ver 0.5 Cope with the change in frisson. The emulation log is not synced among nodes. Now I need to  skip lines till everyone hear about fixed destination.
...
using namespace std;
   struct Node {
    ...
    vector<vector<Node> > oracle(10000, vector<Node>(50));
...
int main(int argc, char* argv[])
{
    ...
    for(unsigned int node=0; node<total_node; node++)
    {
        ...
        if(!fin)
        {
            cout << "This node is not terminating node" << path[node]  << endl;
        }
        else
        {
...
***************************************************************************************************/
/*for(unsigned int snapshot =0; snapshot <10 ; snapshot++)
    {
        stringstream out1;   
        out1 << snapshot;
        string path = "snapshot" + out1.str() + ".txt";
...
//    for(int instance = 0; instance < timestamp ; instance++)    
	...
	//Go through all the node to build ajd matrix
        for(unsigned int node=0; node<total_node; node++)
        {
            ...
            //cout << "vertex " << node << "table looks like";
            bool disconected = true;
            for(unsigned int it = 0; it < total_node; it++){
                //if row is all zero, disconnected
                if(graph[node][it] != 0)
                    disconected = false;
            }
            if(disconected) 
                cout << "Node " << node << "is disconnected" << endl;
        }  
        cout << "Adj matrix table looks like" << endl;
        for(unsigned int row_index = 0; row_index < total_node; row_index++){
            for(unsigned int column =0; column < total_node; column++){
                cout << graph[row_index][column] << " " ;
                //cout << graph[row_index][column] << " ";
            }
            cout << endl;
        }
        ...
        inst_num << instance;
        string instance_path = "result1/shortest_path_count" + inst_num.str() + "_" + fixed_destination_id + ".txt";
        FILE *file = fopen ( instance_path.c_str(), "w" );
        if ( file == NULL ){
            cout << "Could not open file: " << instance_path << endl;
            exit (1);
        }
        ...
        //for(int starter = 0; starter < total_node; starter++){
            ...
            dijkstra(graph, fixed_destination);
