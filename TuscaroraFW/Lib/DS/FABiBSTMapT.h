 ...
 *	The map allows searching with a given KEY or a given DATA.
 *	Search, insertion, erase are O(log n) both in average and in worst cases.
 *	The underlying data structure is two AVLBinarySearchTree's.
 *	FIteratorRef1, RIteratorRef1 are the forward and reverse iterators using the first map using KEY as the key.
 *	FIteratorRef2 and RIteratorRef2 are the forward and reverse iterators using the second map using DATA as the key.
 *	First() and Second() returns KEY and DATA, respectively, for all iterators.
 *
 *
...
#ifndef FABiBST_MAP2T_H_
#define FABiBST_MAP2T_H_
...
public:
	...
	template<class U1, class U2, class U3, class U4> friend class FABiBSTMapT;
...
private:
	...
	FAAVLBST_T< BiMapElement<KEY,DATA>*, MapElementComparator_KEY<BiMapElement<KEY,DATA>*, KEY, KEYCOMPARATOR > >  *myBST1_ptr;
	FAAVLBST_T< BiMapElement<KEY,DATA>*, MapElementComparator_DATA<BiMapElement<KEY,DATA>*, DATA, DATACOMPARATOR > > *myBST2_ptr;
	class IteratorRef1 {
	protected:
...
public:
	...
	bool Insert(const KEY &_key, const DATA &_data){
		...
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node1 = myBST1_ptr->Search(new_element);
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node2 = myBST2_ptr->Search(new_element);
		...
		if(true) {//New data
			if(myBST1_ptr->Insert (new_element)){
				if(myBST2_ptr->Insert (new_element)){
					return true;
				}
				else {
					if(myBST1_ptr->DeleteElement (myBST1_ptr->Search(new_element) ) ) {
						delete new_element;
						return false;
					}
	...
	/*const DATA& operator [] (KEY _key){
		...
		AVLBSTElement< BiMapElement<KEY,DATA>* > *element = myBST1_ptr->Search(onlyKeyElement);
		if(element){
			return element->GetData()->GetData();
		}
		else {
			myBST1_ptr->Insert (onlyKeyElement);
			AVLBSTElement< BiMapElement<KEY,DATA>* > *element = myBST1_ptr->Search(onlyKeyElement);
			return element->GetData().GetData();
		}
	};*/
	...
	Iterator1 GetIterator1(Iterator2 &it2){
		...
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node2 = myBST1_ptr->Search(&new_element);
		return(Iterator1(node2,myBST2_ptr));
	};
	Iterator2 GetIterator2(Iterator1 &it1){
		BiMapElement<KEY,DATA> new_element(it1->First(),it1->Second());
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node1 = myBST2_ptr->Search(&new_element);
		return(Iterator1(node1,myBST2_ptr));
	};


	bool Erase1(const KEY &_key){
		BiMapElement<KEY,DATA> onlyKeyElement(_key);
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node1 = myBST1_ptr->Search(&onlyKeyElement);
		if (node1){
			AVLBSTElement< BiMapElement<KEY,DATA>* > *node2 = myBST2_ptr->Search(node1->GetData());
			...
			if(myBST1_ptr->DeleteElement(node1) && myBST2_ptr->DeleteElement(node2)){
				delete map_element;
				return true;
			}
	...
	bool Erase1(Iterator1& it){
		...
		bool test1 = myBST1_ptr->DeleteElement(it_temp.node);
		bool test2 = myBST2_ptr->DeleteElement(node2);
		assert(test1 == test2);
	...
	bool Erase2(Iterator2& it){
		...
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node2 = myBST1_ptr->Search(&new_element);
		BiMapElement<KEY,DATA>* existing_element = node2->GetData();
		bool test1 = myBST2_ptr->DeleteElement(it_temp.node);
		bool test2 = myBST1_ptr->DeleteElement(node2);
		assert(test1 == test2);
		delete (existing_element);
		return test1;
	};
	...
	/*bool Erase(Reverse_Iterator1 it){
		return(myBST1_ptr->DeleteElement(it.node));
	};*/
	uint32_t Size(){
		return(myBST1_ptr->Size());
	};


	//Iterator1 Related
	Iterator1 Begin1(){
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node = myBST1_ptr->Begin();
		Iterator1 myit(node,myBST1_ptr);
		return (myit);
	};
	Reverse_Iterator1 RBegin1(){
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node = myBST1_ptr->End();
		Reverse_Iterator1 myit(node,myBST1_ptr);
		return (myit);
	};
	Iterator1 End1(){
	...
	Iterator1 Find1(const KEY& _key){
		BiMapElement<KEY,DATA> onlyKeyElement(_key);
		AVLBSTElement< BiMapElement<KEY,DATA>* > *element = myBST1_ptr->Search(&onlyKeyElement);
		Iterator1 myit(element,myBST1_ptr);
		return (myit);
	};

	Iterator2 Begin2(){
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node = myBST2_ptr->Begin();
		Iterator2 myit(node,myBST2_ptr);
		return (myit);
	};
	Reverse_Iterator2 RBegin2(){
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node = myBST2_ptr->End();
		Reverse_Iterator2 myit(node,myBST2_ptr);
		return (myit);
	};
	Iterator2 End2(){
		AVLBSTElement< BiMapElement<KEY,DATA>* > *node = NULL;
	...
	Iterator2 Find2(const DATA& _data){
		...
		onlyKeyElement.data = _data;
		AVLBSTElement< BiMapElement<KEY,DATA>* > *element = myBST2_ptr->Search(&onlyKeyElement);
		Iterator2 myit(element,myBST2_ptr);
		return (myit);
	};

	FABiBSTMapT(uint32_t max_size){
		myBST1_ptr = new FAAVLBST_T< BiMapElement<KEY,DATA>*, MapElementComparator_KEY<BiMapElement<KEY,DATA>*, KEY, KEYCOMPARATOR > >(max_size);
		myBST2_ptr = new FAAVLBST_T< BiMapElement<KEY,DATA>*, MapElementComparator_DATA<BiMapElement<KEY,DATA>*, DATA, DATACOMPARATOR > >(max_size);

	}
	FABiBSTMapT() = delete;// will never be generated
	~FABiBSTMapT(){
		if(myBST1_ptr) delete myBST1_ptr;
		if(myBST21_ptr) delete myBST1_ptr;
	}

};
#endif //FABiBST_MAP2T_H_
